...
const StokConsignmentPage = () => {
  ...
  // Fetch monitoring list specifically for Consignment (Stock Class = 'C')
  const fetchConsignmentData = async () => {
    ...
    try {
      const params = {
        ...
        stock_class: 'C', // HARDCODED for Consignment (STOCK-COGS data)
        status,
        sortBy,
        sortOrder
      };
  ...
  }, [selectedUpload, page, pageSize, searchText, warehouse, vendor, stockType, status, sortBy, sortOrder]);
  ...
  return (
    <Space direction="vertical" size="large" style={{ width: '100%' }}>
      <div>
        <Title level={2}>Stok Consignment</Title>
        <Paragraph>Lihat dan filter seluruh data inventaris barang konsinyasi (berasal dari sheet STOCK-COGS / Class C).</Paragraph>
      </div>
...
export default StokConsignmentPage;
